...
function WayHandlers.ferries(profile,way,result,data)
  ...
  if route then
    local route_weight = profile.route_weights[route]
    if route_weight and route_weight[1] > 0 then
     ...
     result.forward_rate = route_weight[1]
     result.backward_rate = route_weight[1]
     if route_weight[2] then
       result.forward_speed = route_weight[2]
       result.backward_speed = route_weight[2]
     end
    end
  end
end
...
function WayHandlers.speed(profile,way,result,data)
  ...
  if speed then
    ...
    result.forward_rate = speed[1]
    result.backward_rate = speed[1]
    result.forward_speed = speed[2]
    result.backward_speed = speed[2]
  else
    -- Set the avg speed on ways that are marked accessible
    if profile.access_tag_whitelist[data.forward_access] then
      result.forward_speed = profile.default_speed
      result.forward_rate = profile.default_weight
    ...
    if profile.access_tag_whitelist[data.backward_access] then
      result.backward_rate = profile.default_weight
...
function WayHandlers.surface(profile,way,result,data)
  ...
  if surface and profile.surface_speeds[surface] then
    result.forward_rate = math.min(profile.surface_speeds[surface][1], result.forward_rate)
    result.backward_rate = math.min(profile.surface_speeds[surface][1], result.backward_rate)
    result.forward_speed = math.min(profile.surface_speeds[surface][2], result.forward_speed)
    result.backward_speed = math.min(profile.surface_speeds[surface][2], result.backward_speed)
  end
  if tracktype and profile.tracktype_speeds[tracktype] then
    result.forward_rate = math.min(profile.tracktype_speeds[tracktype][1], result.forward_rate)
    result.backward_rate = math.min(profile.tracktype_speeds[tracktype][1], result.backward_rate)
    result.forward_speed = math.min(profile.tracktype_speeds[tracktype][2], result.forward_speed)
    result.backward_speed = math.min(profile.tracktype_speeds[tracktype][2], result.backward_speed)
  end
  if smoothness and profile.smoothness_speeds[smoothness] then
    result.forward_rate = math.min(profile.smoothness_speeds[smoothness][1], result.forward_rate)
    result.backward_rate = math.min(profile.smoothness_speeds[smoothness][1], result.backward_rate)
    result.forward_speed = math.min(profile.smoothness_speeds[smoothness][2], result.forward_speed)
    result.backward_speed = math.min(profile.smoothness_speeds[smoothness][2], result.backward_speed)
  end
end
...
function WayHandlers.penalties(profile,way,result,data)
  ...
  if result.forward_rate <= 0 and result.forward_speed > 0 then
    result.forward_rate = (result.forward_speed * forward_penalty) / 3.6
  end
  if  result.backward_rate <= 0 and result.backward_speed > 0 then
    result.backward_rate = (result.backward_speed * backward_penalty) / 3.6
  end
  if result.duration > 0 and result.forward_rate <= 0 then
    result.forward_rate = result.duration / forward_penalty
  end
  if result.duration > 0 and result.backward_rate <= 0 then
    result.backward_rate = result.duration / backward_penalty
  end
end
        ...
        for k,v in pairs(profile.speed_path) do
            ...
            if tag then
                if v == 0 then
                    ...
                    if v[tag] ~= nil then
                        result.forward_rate = result.forward_rate * v[tag][1]
                        result.backward_rate = result.backward_rate * v[tag][1]
                        result.forward_speed = math.floor(result.forward_speed * v[tag][2])
                        result.backward_speed = math.floor(result.backward_speed * v[tag][2])
                    end
                end
            end
        end
        return (result.forward_rate > 0 or result.backward_rate > 0)
    end
